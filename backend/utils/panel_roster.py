...
# --- v3: four secondary families, chosen against clinicians ----------------
#
# The bake-off (10 items x 7 models x 3 fixed personas, 210/210 calls returned) went to
# the research group as a blinded rating sheet, and every candidate's ratings came back
# judged defensible. That answers the question the sheet was built to ask — whether a
# minority rating is a clinical position or simply wrong — and it is the reason these
# four are seated rather than the one incumbent.
#
# It does **not** answer whether a model belongs in the instrument, and two candidates
# are excluded on grounds that have nothing to do with rating quality:
#
#   google/gemini-3.1-pro-preview  excluded. A preview id can change or be withdrawn
#                                  under a live instrument, which would silently alter
#                                  what a distribution was generated by. Unfit
#                                  regardless of how it rates.
#   z-ai/glm-5.2                   excluded. Entered as a negative control for echoing
#                                  the primary. It did dissent on bake-off item 1, so
#                                  "echo" is not settled — but a control admitted to the
#                                  panel on the strength of the run it was the control
#                                  for is circular. Re-test it as a candidate first.
#
# One model per seat, not one model per arm. That is a real trade: with n=1 per family a
# single seat's rating can no longer be attributed to its model rather than its persona.
# The roster does not exist to estimate model effects, it exists to produce a calibrated
# distribution, and four families reduce the chance that a silent one takes all the
# disagreement with it — the failure that actually happened on 2026-08-12.
MODEL_SECONDARY = os.getenv("ORACLE_MODEL_SECONDARY", "anthropic/claude-opus-4.6")
MODEL_SECONDARY_2 = os.getenv("ORACLE_MODEL_SECONDARY_2", "x-ai/grok-4.6")
MODEL_SECONDARY_3 = os.getenv("ORACLE_MODEL_SECONDARY_3", "moonshotai/kimi-k3")
# Rated hardest against the primary and had one call fail in the July probe. Kept
# because the clinicians read its dissent as defensible; `panel_runner` retries, and a
# seat that still fails is caught by `model_family_silent`.
MODEL_SECONDARY_4 = os.getenv("ORACLE_MODEL_SECONDARY_4", "deepseek/deepseek-v4-pro")
...
# v3 adds a fourth secondary seat and gives each one its own model. Seat 3 is the new
# one, taken from the setting trio (1 community / 2 academic / 3 high-volume urban)
# rather than from the seniority pair, because 4-vs-5 is early- vs late-career and
# splitting a pair that already differs on one axis adds a second axis to it. Seat 1
# stays on the primary so the trio is not entirely secondary.
SEAT_MODELS: dict[int, str] = {
    2: MODEL_SECONDARY,  # em_academic — the incumbent, kept for continuity with v2
    3: MODEL_SECONDARY_2,  # em_high_volume_urban
    4: MODEL_SECONDARY_3,  # em_early_career
    7: MODEL_SECONDARY_4,  # hospitalist, paired with 6 general_internist
}

# Derived, not authored. Kept because `scripts/model_bakeoff.py` samples the seats the
# roster actually swaps, so that it tests the arm it is choosing models for.
SECONDARY_MODEL_SEATS: frozenset[int] = frozenset(SEAT_MODELS)
    ...
    for i, (persona_id, role, persona) in enumerate(_ROSTER, start=1):
        ...
        roster.append(
            Panelist(
                ...
                role=role,
                persona=persona,
                model=SEAT_MODELS.get(i, MODEL_PRIMARY),
            )
        )
    return roster
