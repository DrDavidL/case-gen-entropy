...
## ADR-022 — Four secondary model families, chosen against clinicians rather than by capability.

**Status:** ACCEPTED (2026-08-14) · BUILT · Roster `v3` · Extends `ADR-018`

**Context.** `ADR-018` split the roster across two families because fifteen personas on one model
are fifteen draws from one posterior. On 2026-08-12 the secondary family died silently: every seat
on `anthropic/claude-sonnet-5` began returning 400 on the strict schema, the panel still produced a
well-formed distribution from twelve seats, and *every* dissenting vote on every item checked had
been coming from the three seats that were now gone. Losing a family did not cost three opinions
out of fifteen; it cost all of the measured disagreement and turned items with 77-80% agreement
into items with 100%, which cannot separate learners.

Across 36 live runs, 24 would have had zero entropy from the primary family alone. Model
composition is therefore not a configuration detail, and the previous choice had been made from a
capability matrix of one.

**Decision.** Four secondary seats, one model each: `anthropic/claude-opus-4.6` (seat 2),
`x-ai/grok-4.6` (3), `moonshotai/kimi-k3` (4), `deepseek/deepseek-v4-pro` (7). Eleven seats remain
on `openai/gpt-5.6-sol`.

Chosen from the bake-off (`scripts/model_bakeoff.py`, 10 items x 7 models x 3 fixed personas,
210/210 calls returned). The blinded rating sheet went to the research group, which judged every
candidate's ratings clinically defensible. **That is the finding that seats these four**, and it is
the finding the harness was built to obtain: a model that dissents because it is weaker at medicine
manufactures spread that looks like discrimination while handing learners partial credit for wrong
answers, and only a clinician reading the reasoning can tell the two apart.

Seat 3 is the new secondary, taken from the setting trio (community / academic / high-volume urban)
rather than the seniority pair, because 4-vs-5 is early- vs late-career and splitting a pair that
already differs on one axis adds a second axis to it. Seats 8-15 stay on one model for the reason
`ADR-018` gives: where a persona contrast *is* the measurement, a model difference would confound
it.

**Rejected, on grounds unrelated to rating quality.** The research group cleared all seven
candidates, and two are still excluded:

- `google/gemini-3.1-pro-preview` — a preview id. It can change or be withdrawn under a live
  instrument, which would silently alter what a distribution was generated by. The Oracle is a
  measurement instrument before it is a model consumer, so this is disqualifying regardless of how
  it rates.
- `z-ai/glm-5.2` — entered as a negative control for echoing the primary. It did dissent on
  bake-off item 1, so "echo" is no longer settled, but admitting a control to the panel on the
  strength of the run it was the control for is circular. Re-test it as a candidate.

**What this costs.** One model per seat means n=1 per secondary family, so a seat's rating can no
longer be attributed to its model rather than its persona. Accepted: the roster exists to produce a
calibrated distribution, not to estimate model effects, and four families make it far less likely
that one silent family takes all the disagreement with it. `model_family_silent` still fires per
model, and its wording no longer assumes exactly two.

`deepseek/deepseek-v4-pro` had one call fail in the July probe and rated hardest against the
primary. Kept because the clinicians read its dissent as defensible; `panel_runner` retries, and a
seat that still fails is caught by the flag.

**Verified 2026-08-14**, one live call per secondary seat through `panel_runner._rate_once`: all
four answer under ZDR with the strict schema (`+2`, `+1`, `+2`, `+2`; 3-25s). That check exists
because this is exactly how Sonnet 5 was lost, and a roster change is the moment to run it.

**Consequences.** Roster `v3`. Distributions generated under `v2` are not comparable seat-for-seat
and must not be pooled with `v3` — `panel_roster_version` is stamped on every run so the boundary
is visible in the data. Re-running the Oracle on cases that already carry `v2` distributions is a
